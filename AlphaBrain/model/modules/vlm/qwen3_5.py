...
class _QWen3_5_VL_Interface(nn.Module):
    ...
    def __init__(self, config: Optional[dict] = None, **kwargs):
        ...
        # 框架内部自动控制：自包含checkpoint场景下仅创建架构骨架，权重由框架统一加载
        use_meta_device = qwenvl_config.get("_meta_device_init", False)
        attn_impl = qwenvl_config.get("attn_implementation", "flash_attention_2")
        print(f"[vlm_init] attn_implementation={attn_impl}")
        if use_meta_device:
            from transformers import AutoConfig
            qwen_config = AutoConfig.from_pretrained(model_id, trust_remote_code=True)
            qwen_config.attn_implementation = attn_impl
            with torch.device("meta"):
                model = Qwen3_5ForConditionalGeneration(qwen_config)
            model = model.to_empty(device="cpu")
            # Rebuild non-persistent RoPE buffers lost after to_empty
            for _name, _module in model.named_modules():
                if not hasattr(_module, "inv_freq"):
                    continue
                if hasattr(_module, "rope_init_fn") and hasattr(_module, "config"):
                    # Text RoPE (Qwen3VLTextRotaryEmbedding)
                    _inv_freq, _attn_scale = _module.rope_init_fn(_module.config, device="cpu")
                    _module.inv_freq = _inv_freq
                    if hasattr(_module, "attention_scaling"):
                        _module.attention_scaling = _attn_scale
                else:
                    # Vision RoPE (Qwen3VLVisionRotaryEmbedding): simple formula
                    _dim = _module.inv_freq.shape[0] * 2
                    _module.inv_freq = 1.0 / (10000.0 ** (torch.arange(0, _dim, 2, dtype=torch.float) / _dim))
            print(f"[meta_device_init] Created Qwen3.5 model via meta device, model_id={model_id}")
        else:
